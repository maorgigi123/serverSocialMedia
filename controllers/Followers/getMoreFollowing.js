...
const GetMoreFollowing = () => async (req, res) => {
    try {
        ...
        // Find friends, populate the "following" field, and paginate the results
        const friends = await FriendsSchema.find({ follower: userID })
            .populate({
                path: 'following',
                options: {
        ...
        const friendsWithMutualStatus = await Promise.all(friends.map(async (_user) => {
            if (_user.follower && _user.follower._id) {  // Check if follower exists and has an _id
                const friendRecord = await FriendsSchema.findOne({ follower: CurrentUserID, following: _user.following._id });
...
module.exports = GetMoreFollowing;
